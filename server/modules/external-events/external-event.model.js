...
const externalEventSchema = new mongoose.Schema(
  {
    ...
    universityName: { type: String, required: [true, "University name is required"] },
    venue:        String,
    category:     { type: String, enum: ["hackathon", "workshop", "webinar", "cultural", "sports", "conference", "competition"], required: true },
    date:         { type: Date, required: true },
    registrationLink: {
      type: String, required: [true, "Registration link is required"],
      validate: { validator: (v) => /^https?:\/\/.+/.test(v), message: "Must be a valid URL" },
    },
