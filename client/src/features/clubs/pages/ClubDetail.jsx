...
export default function ClubDetail() {
  ...
  const handleOpenChat = async () => {
    ...
    try {
      ...
      const res = await createClubChat(id, `${club.name} Chat`);
  ...
  const handlePromote = async () => {
    ...
    try {
      await assignCoordinator(id, promotingMember.userId, coordCategory);
      setMembers((prev) =>
        prev.map((m) =>
  ...
  const handleDemote = async (userId) => {
    ...
    try {
      await removeCoordinator(id, userId);
      setMembers((prev) =>
        prev.map((m) =>
          String(m.userId?._id || m.userId) === String(userId)
  ...
  const handlePostAnnouncement = async (e) => {
    ...
    setAnnPosting(true);
    try {
      const res = await createAnnouncement(id, annForm);
